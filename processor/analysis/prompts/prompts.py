...
任务：基于 JS 代码和 Level 2 参数名线索，提取完整请求信息并评估 API 危险性。

【规则】
1. Level 2 参数名仅供参考，以代码为准；优先提取真实值，无值时推测(id=1等)；忽略单字母变量
2. Path：结合上下文尝试拼接，否则直接输出
3. Method：从代码提取(GET/POST)，无法提取时输出""
4. 危险性评估(dangerous)：若未授权测试请求可能对系统造成实质影响则标记 true
   - true：写操作(PUT/DELETE的增删改提交)、批量操作、文件上传、资金操作、权限变更
   - false：纯查询/只读接口、常规认证接口

【输出格式】纯 JSON：
{"path": "api path", "method": "GET/POST或空", "params": "k1=v1,k2=v2", "dangerous": true或false}
"""
