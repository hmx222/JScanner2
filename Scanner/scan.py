...
from Scanner.utils import (
    classify_url, is_api_path_blacklisted, _is_skip_ext,
    BATCH_SIZE, BATCH_SLEEP, API_MIN_LENGTH
)
    ...
    async def _process_ai_batch(self, batch_all_next_paths_with_source, batch_scan_info_list, batch_next_urls):
        ...
        if getattr(self.args, 'fastscan', False) and batch_next_urls:
            ...
            if urls_to_scan:
                ...
                print(f"🌐 [FastScan] {len(urls_to_scan)} URLs → {dup_count} duplicates → {len(qualified_api_paths)} qualified")
